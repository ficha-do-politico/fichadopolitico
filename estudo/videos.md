# Vídeos de referência

Fontes usadas nos resumos de `todos-os-cargos.md` e `tres-poderes.md`.

- **Como funcionam os Três Poderes do Brasil | EXAMINANDO**
  Canal: EXAMINANDO
  https://www.youtube.com/watch?v=9ltvYAN7R74

- **Entenda o sistema político no Brasil - OS 3 PODERES - de um jeito SIMPLES**
  Canal: não confirmado (YouTube não expôs o nome no fetch automático — conferir manualmente)
  https://www.youtube.com/watch?v=90N6xRU58Vo

- **Explicando Todos os Cargos Políticos em 10 Minutos**
  Canal: não confirmado (YouTube não expôs o nome no fetch automático — conferir manualmente)
  https://www.youtube.com/watch?v=kwajt0nb91c
